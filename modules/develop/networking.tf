...
resource "aws_subnet" "private_subnet" {
 count = "develop" == "develop" ? length(data.aws_availability_zones.available[0].names) : 0
 vpc_id = aws_vpc.vpc[0].id
 ...
 tags = {
  Name        = join("-", ["develop", element(data.aws_availability_zones.available[0].names, count.index), "private-subnet"])
  Environment = "develop"
 }
}
/* Public subnet */
resource "aws_subnet" "public_subnet" {
 ...
 tags = {
  Name        = join("-", ["develop", element(data.aws_availability_zones.available[0].names, count.index), "public-subnet"])
  Environment = "develop"
 }
}
